...
namespace RestCsharp.Datos
{
   public  class Dserealizacion
    {
        public bool Insertar_Serializacion(Lserializacion parametros)
        {
            try
            {
        ...
        public void mostrarCompDefecto(ref string comprobante)
        {
            try
            {
                CONEXIONMAESTRA.abrir();
                SqlCommand da = new SqlCommand("mostrarCompDefecto", CONEXIONMAESTRA.conectar);
                comprobante = da.ExecuteScalar().ToString();
